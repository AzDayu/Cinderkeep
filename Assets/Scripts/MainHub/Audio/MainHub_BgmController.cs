...
{
    // Main hub BGM ?꾩슜 而⑦듃濡ㅻ윭?낅땲??
    // ????묒뾽 ?곸뿭怨??ㅼ젣 寃뚯엫 ?곸뿭??遺꾨━?섍린 ?꾪빐 Cinderkeep_Game?먯꽌???먮룞?쇰줈 硫덉땅?덈떎.
    // Main_Lobby?먯꽌 ?쒖옉??BGM? 罹먮┃???좏깮/媛쒖씤 ?묒뾽??怨듬룞 ?묒뾽?ㅺ퉴吏 ?좎??⑸땲??
    public sealed class MainHub_BgmController : MonoBehaviour
    {
        ...
        public void SetReferences(AudioSource bgmSource, AudioClip[] bgmPlaylist, float volume)
        {
            ...
            // ?꾩옱 Cinderkeep ?덈툕 BGM ?뺤콉: ?대뼡 BGM??異붽??대룄 湲곕낯 蹂쇰ⅷ? 0.4?낅땲??
